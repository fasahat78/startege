/**
 * Create missing concepts for Level 15 and assign them
 * 
 * This script:
 * 1. Creates ConceptCard records for missing concepts with placeholder definitions
 * 2. Assigns all concepts (existing + newly created) to Level 15
 */

import { prisma } from "../lib/db";
import { LEVEL_CONFIGS } from "../lib/levels";

// Level 15 concepts with metadata from CSV
const LEVEL_15_CONCEPTS = [
  {
    name: "What Makes an AI System High-Risk",
    domain: "Domain 2",
    category: "High-Risk AI Systems",
    difficulty: "intermediate",
    notes: "Clarifies high-risk classification logic",
  },
  {
    name: "Annex III High-Risk Use Case Categories (Conceptual)",
    domain: "Domain 2",
    category: "High-Risk AI Systems",
    difficulty: "intermediate",
    notes: "Introduces high-risk scope without memorisation",
  },
  {
    name: "High-Risk vs Non-High-Risk Boundary Cases",
    domain: "Domain 2",
    category: "High-Risk AI Systems",
    difficulty: "intermediate",
    notes: "Reflects real governance ambiguity",
  },
  {
    name: "Lifecycle Obligations Triggered by High-Risk Classification",
    domain: "Domain 2",
    category: "High-Risk AI Systems",
    difficulty: "intermediate",
    notes: "Shows obligation escalation across lifecycle",
  },
  {
    name: "Risk Management Expectations for High-Risk AI",
    domain: "Domain 3",
    category: "Risk Identification & Assessment",
    difficulty: "intermediate",
    notes: "Connects risk governance to formal status",
  },
  {
    name: "Documentation Burden for High-Risk AI Systems",
    domain: "Domain 2",
    category: "High-Risk AI Systems",
    difficulty: "intermediate",
    notes: "Makes governance effort explicit",
  },
  {
    name: "Role of Impact Assessments in High-Risk AI Governance",
    domain: "Domain 3",
    category: "Impact Assessments",
    difficulty: "intermediate",
    notes: "Reinforces assessments as mandatory",
  },
  {
    name: "Accountability for High-Risk AI Systems",
    domain: "Domain 1",
    category: "Governance Structures & Roles",
    difficulty: "intermediate",
    notes: "Raises ownership and accountability stakes",
  },
  {
    name: "Residual Risk Acceptance for High-Risk AI",
    domain: "Domain 3",
    category: "Risk Identification & Assessment",
    difficulty: "intermediate",
    notes: "Introduces formal risk acceptance logic",
  },
  {
    name: "High-Risk AI Obligations vs Limited-Risk Obligations",
    domain: "Domain 2",
    category: "AI Act Obligations & Requirements",
    difficulty: "intermediate",
    notes: "Sharpens regulatory distinctions",
  },
];
...
async function setupLevel15() {
  ...
  for (const conceptData of LEVEL_15_CONCEPTS) {
    try {
  ...
  const levelConfig = LEVEL_CONFIGS.find((c) => c.level === 15);
  if (!levelConfig) {
    throw new Error(`Level 15 config not found`);
  }

  let challenge = await (prisma as any).challenge.findFirst({
    where: { levelNumber: 15 },
  });
  ...
  console.log(`\n💾 Assigning concepts to Level 15...`);
  ...
  console.log(`✅ Assigned ${conceptIds.length} concepts to Level 15`);

  // Step 4: Set up LevelCategoryCoverage
  console.log(`\n📊 Setting up LevelCategoryCoverage...`);

  const categoryCoverage = [
    { categoryName: "High-Risk AI Systems", domainName: "Domain 2", coverageType: "INTRODUCED" },
...
async function main() {
  console.log("🚀 Creating and assigning concepts for Level 15\n");
  ...
  try {
    ...
    console.log("   - Level 15 is ready for testing");
  } catch (error) {
    console.error("\n❌ Error:", error);
    throw error;
  }
