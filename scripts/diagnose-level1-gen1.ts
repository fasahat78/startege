/**
 * Diagnostic script for Level 1 Generation 1
...
async function diagnose() {
  ...
  // Script info
  try {
    const scriptPath = path.join(process.cwd(), "scripts", "diagnose-level1-gen1.ts");
    const scriptStats = fs.statSync(scriptPath);
    console.log(`Script: ${scriptPath}`);
    console.log(`Last Modified: ${scriptStats.mtime.toISOString()}`);
  } catch (e) {
    console.log(`Script: scripts/diagnose-level1-gen1.ts`);
  }
