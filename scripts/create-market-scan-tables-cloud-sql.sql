-- Create Market Scan tables for Cloud SQL Studio
-- Run this script statement by statement in Cloud SQL Studio
-- Each DO block should be run separately if you get errors

-- Step 1: Create ScanJobStatus enum
DO $$ BEGIN
    CREATE TYPE "ScanJobStatus" AS ENUM ('RUNNING', 'COMPLETED', 'FAILED', 'CANCELLED');
EXCEPTION WHEN duplicate_object THEN null;
END $$;

-- Step 2: Create ScanJobType enum
DO $$ BEGIN
    CREATE TYPE "ScanJobType" AS ENUM ('DAILY_NEWS', 'MANUAL', 'SCHEDULED');
EXCEPTION WHEN duplicate_object THEN null;
END $$;

-- Step 3: Create ComplianceImpact enum
DO $$ BEGIN
    CREATE TYPE "ComplianceImpact" AS ENUM ('High', 'Medium', 'Low');
EXCEPTION WHEN duplicate_object THEN null;
END $$;

-- Step 4: Create Sentiment enum
DO $$ BEGIN
    CREATE TYPE "Sentiment" AS ENUM ('positive', 'negative', 'neutral');
EXCEPTION WHEN duplicate_object THEN null;
END $$;

-- Step 5: Create Urgency enum
DO $$ BEGIN
    CREATE TYPE "Urgency" AS ENUM ('breaking', 'high', 'medium', 'low');
EXCEPTION WHEN duplicate_object THEN null;
END $$;

-- Step 6: Create ImpactScope enum
DO $$ BEGIN
    CREATE TYPE "ImpactScope" AS ENUM ('global', 'regional', 'local', 'industry_specific');
EXCEPTION WHEN duplicate_object THEN null;
END $$;

-- Step 7: Create ComplexityLevel enum
DO $$ BEGIN
    CREATE TYPE "ComplexityLevel" AS ENUM ('beginner', 'intermediate', 'advanced', 'expert');
EXCEPTION WHEN duplicate_object THEN null;
END $$;

-- Step 8: Create ScanJob table
...
-- Step 9: Create indexes for ScanJob
...
-- Step 10: Create MarketScanArticle table
...
-- Step 11: Create indexes for MarketScanArticle
