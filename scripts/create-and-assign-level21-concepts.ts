/**
 * Create missing concepts for Level 21 and assign them
 * 
 * This script:
 * 1. Creates ConceptCard records for missing concepts with placeholder definitions
 * 2. Assigns all concepts (existing + newly created) to Level 21
 */

import { prisma } from "../lib/db";
import { LEVEL_CONFIGS } from "../lib/levels";

// Level 21 concepts with metadata from CSV
const LEVEL_21_CONCEPTS = [
  {
    name: "Governing AI Under Uncertainty",
    domain: "Domain 4",
    category: "Advanced Governance Scenarios",
    difficulty: "advanced",
    notes: "Establishes uncertainty as a governance condition",
  },
  {
    name: "Conflicting Governance Objectives",
    domain: "Domain 4",
    category: "Advanced Governance Scenarios",
    difficulty: "advanced",
    notes: "Introduces value trade-offs",
  },
  {
    name: "Decision-Making with Incomplete Evidence",
    domain: "Domain 4",
    category: "Advanced Governance Scenarios",
    difficulty: "advanced",
    notes: "Reflects real governance constraints",
  },
  {
    name: "Escalation When No Clear Policy Exists",
    domain: "Domain 4",
    category: "Advanced Governance Scenarios",
    difficulty: "advanced",
    notes: "Tests judgement beyond rules",
  },
  {
    name: "Managing Trade-Offs Across Multiple Risks",
    domain: "Domain 4",
    category: "Advanced Governance Scenarios",
    difficulty: "advanced",
    notes: "Reinforces portfolio thinking",
  },
  {
    name: "Acceptable Risk vs Unacceptable Harm",
    domain: "Domain 4",
    category: "Advanced Governance Scenarios",
    difficulty: "advanced",
    notes: "Sharpens ethical judgement",
  },
  {
    name: "Stress-Testing Compliance Frameworks with Edge Cases",
    domain: "Domain 1",
  ...
  {
    name: "Defending Governance Decisions After the Fact",
    domain: "Domain 1",
    category: "Algorithmic Accountability & Assurance",
    difficulty: "advanced",
    notes: "Introduces retrospective scrutiny",
  },
  {
    name: "Fairness Trade-Offs in High-Stakes Decisions",
    domain: "Domain 3",
    category: "Bias Fairness & Model Risk",
    difficulty: "advanced",
    notes: "Applies fairness under pressure",
  },
  {
    name: "Responding to Regulatory Scrutiny in Ambiguous Cases",
    domain: "Domain 4",
    category: "Enforcement Oversight & Remedies",
    difficulty: "advanced",
    notes: "Tests regulatory judgement",
  },
];
...
async function setupLevel21() {
  ...
  for (const conceptData of LEVEL_21_CONCEPTS) {
    try {
  ...
  const levelConfig = LEVEL_CONFIGS.find((c) => c.level === 21);
  if (!levelConfig) {
    throw new Error(`Level 21 config not found`);
  }

  let challenge = await (prisma as any).challenge.findFirst({
    where: { levelNumber: 21 },
  });
  ...
  console.log(`\n💾 Assigning concepts to Level 21...`);
  ...
  console.log(`✅ Assigned ${conceptIds.length} concepts to Level 21`);

  // Step 4: Set up LevelCategoryCoverage
  console.log(`\n📊 Setting up LevelCategoryCoverage...`);

  const categoryCoverage = [
    { categoryName: "Advanced Governance Scenarios", domainName: "Domain 4", coverageType: "INTRODUCED" },
...
async function main() {
  console.log("🚀 Creating and assigning concepts for Level 21\n");
  ...
  try {
    ...
    console.log("   - Level 21 is ready for testing");
  } catch (error) {
    console.error("\n❌ Error:", error);
    throw error;
  }
