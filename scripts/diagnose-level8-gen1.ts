/**
 * Diagnostic script for Level 8 Generation 1
 * Tests if the categoryIdMap fix resolves the issues
 */
...
async function diagnose() {
  ...
  console.log("🔍 DIAGNOSTIC INSTRUMENTATION - Level 8 Generation 1");
  console.log("=".repeat(60));
  
  const scriptPath = path.join(process.cwd(), "scripts", "diagnose-level8-gen1.ts");
  try {
    const scriptStats = fs.statSync(scriptPath);
    console.log(`Script: ${scriptPath}`);
    console.log(`Last Modified: ${scriptStats.mtime.toISOString()}`);
  } catch (e) {
    console.log(`Script: scripts/diagnose-level8-gen1.ts`);
  }
  ...
  console.log(`📋 Categories for Level 8:`);
  console.log(`   Count: ${requiredCategoryIds.length} categories`);
  Object.entries(categoryIdMap).forEach(([id, name]) => {
    console.log(`     - ${name} (${id})`);
  });
  console.log("");
  ...
  if (validation.warnings.length > 0) {
    console.log(`\n   ⚠️  WARNINGS:`);
    validation.warnings.slice(0, 5).forEach((w, i) => {
      console.log(`     ${i + 1}. ${w}`);
    });
  ...
  const allCovered = concepts.every((c: any) => usedConcepts.has(c.id));
  console.log(`   All concepts covered: ${allCovered ? "✅ YES" : "❌ NO"}`);
  
  if (!allCovered) {
    const missing = concepts.filter((c: any) => !usedConcepts.has(c.id));
    console.log(`   Missing concepts:`);
    missing.forEach((c: any) => {
      console.log(`     - ${c.name || c.concept} (${c.id})`);
    });
  }

  console.log("\n" + "=".repeat(60));
  console.log(`📊 SUMMARY: Level 8 Generation 1 ${validation.isValid && allCovered ? "✅ PASSES" : "❌ FAILS"}`);
  console.log("=".repeat(60));
