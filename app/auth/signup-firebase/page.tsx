...
function SignUpFirebaseForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast("Account created successfully! Redirecting...", "success", 2000);

      // Get ID token
      const idToken = await getIdToken(true); // Force refresh to get updated token

      if (!idToken) {
        throw new Error("Failed to get ID token");
      }

      PersistentLogger.log("===== STARTING SIGNUP VERIFY =====");
      PersistentLogger.log("Name", name);
      PersistentLogger.log("ID Token length", idToken.length);
      
      console.log("[CLIENT] ===== STARTING SIGNUP VERIFY =====");
      ...
      // This avoids opaqueredirect issues and ensures server sets the session cookie
      ...
      redirectInput.value = "/onboarding/persona";
      ...
      PersistentLogger.error("Sign up error", error);
  ...
  const handleOAuthSignUp = async (provider: "google" | "apple") => {
    ...
    try {
      ...
      PersistentLogger.log("===== STARTING OAUTH SIGNUP VERIFY =====");
      PersistentLogger.log("Provider:", provider);
      PersistentLogger.log("ID Token length", idToken.length);
      
      console.log("[CLIENT] ===== STARTING OAUTH SIGNUP VERIFY =====");
      ...
      let errorMessage = `Failed to sign up with ${provider === "google" ? "Google" : "Apple"}`;
      ...
      PersistentLogger.error("OAuth sign up error", error);
      setError(errorMessage);
      toast(errorMessage, "error");
      ...
      <div className="max-w-md w-full bg-card rounded-lg shadow-card p-8 border-2 border-border">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">
            Sign up to start your AI Governance journey
          </p>
        </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-muted-foreground">
            ...
            <Link href="/auth/signin-firebase" className="text-accent hover:text-accent/80 font-medium">
              Sign in
            </Link>
          </p>
        </div>
